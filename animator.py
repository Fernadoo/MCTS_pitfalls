...
class Animation:
    def __init__(self, agents, my_map, starts, goals, history, FPS):
        ...
        for i in range(len(self.my_map)):
            for j in range(len(self.my_map[0])):
                ...
                elif self.my_map[i][j] in Marker.ACCESSIBLE:
                    self.patches.append(
                        Rectangle((i - 0.5, j - 0.5),
                                  1, 1,
                ...
                if self.my_map[i, j] == Marker.PITFALL:
    ...
    def init_func(self):
        ...
        if getattr(self, 'already_saved', None) is None or not self.already_saved:
            self.fig.savefig('layout.png', dpi=300)
